...
void HCSR04_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
  if(htim == htim_hcsr04)
  {
    uint16_t stoptime = __HAL_TIM_GetCompare(htim_hcsr04, HCSR04_STOP_CHANNEL);
    uint16_t starttime = __HAL_TIM_GetCompare(htim_hcsr04, HCSR04_START_CHANNEL);

    uint16_t time = stoptime-starttime;
